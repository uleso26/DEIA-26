...
# Load JSON from the active data source
def load_json(path: Path) -> Any:
    with path.open("r", encoding="utf-8") as handle:
        return json.load(handle)


# Write JSON to the local runtime storage
...
# Load collection from MongoDB from the active data source
...
# Load collection with backend from the active data source
...
# Load collection from the active data source
...
# Run a Neo4j query through the configured backend or fallback path
...
# Run a Neo4j query and report which backend satisfied it
...
# Summarize backend availability and the active retrieval configuration
...
# Load graph from the active data source
...
# Load retrieval manifest from the active data source
...
# Build lexical index for the downstream execution path
...
# Split retrieval documents into bounded chunks for indexing
...
# Build dense index for the downstream execution path
...
# Build chroma index for the downstream execution path
...
# Search lexical index and return the best matches
...
# Search dense index and return the best matches
