...
# Clean graph properties so Neo4j receives only serializable values
def _clean_properties(properties: dict[str, Any]) -> dict[str, Any]:
    return {key: value for key, value in properties.items() if value is not None}


# Clear graph from the in-memory runtime state
def _clear_graph(tx: Any) -> None:
    tx.run("MATCH (n) DETACH DELETE n").consume()


# Write the current graph payload into Neo4j
...
# Sync to Neo4j into the target backend
